    ...
    describe("calculatePositionClaimableReward()", async function () {
      ...
      describe("calculatePositionClaimableReward() With Vesting", async function () {
        ...
        it("should update additional rewards when we update the commission after position is maturing/matured", async function () {
          ...
          await hydraDelegation.connect(delegatedValidator).setPendingCommission(90);

          // commit epochs to distribute rewards
          await commitEpochs(systemHydraChain, hydraStaking, [delegatedValidator], 5, this.epochSize, WEEK * 5);
          ...
          await hydraDelegation.connect(delegatedValidator).applyPendingCommission();
        ...
        it("should not add additional rewards when we update the commission to 100 after position is matured", async function () {
          ...
          await hydraDelegation.connect(delegatedValidator).setPendingCommission(100);
